...
#    expect_error(isDefined(ds.test_env$connections, "E$A"), "The input object E$A is not defined in discordant1, discordant2, discordant3!", fixed=TRUE)
    ...
    expect_length(err, 3)
    expect_equal(err$discordant1, "Execution failed: Error in base::exists(\"A\", E) : object 'E' not found", fixed=TRUE)
    expect_equal(err$discordant2, "Execution failed: Error in base::exists(\"A\", E) : object 'E' not found", fixed=TRUE)
    expect_equal(err$discordant3, "Execution failed: Error in base::exists(\"A\", E) : object 'E' not found", fixed=TRUE)
})
