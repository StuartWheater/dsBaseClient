...
# CALL THE MAIN SERVER SIDE FUNCTION
  ...
  tryCatch(DSI::datashield.assign(datasources, newobj, calltext), error = function(e) { assign("error", e, parent.frame(4)) })
  if (is.null(error)) {
      return(list(successful = TRUE, outcome.message = 'Success', studyside.messages = list()))
  } else {
      errors <- DSI::datashield.errors()
      if (is.null(errors)) {
          return(list(successful = FALSE, outcome.message = error, studyside.messages = list()))
      } else {
          return(list(successful = FALSE, outcome.message = 'Server problems', studyside.messages = errors))
      }
  }
}
#ds.Boole
